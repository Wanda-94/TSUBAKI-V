...
void InitWindow()
{
	...
	g_window = glfwCreateWindow(screen_width, screen_height, window_name, nullptr, nullptr);
}
...
void CreateVulkanInstance()
{
	...
	std::cout << "glfw need extension "+std::to_string(glfw_extension_count) << std::endl;
	for (int i = 0; i < glfw_extension_count; i++)
	{
		std::cout << "	" + std::string(glfw_extensions[0]) << std::endl;
	}
	//should i check extension is available?
	//uint32_t total = 12;
	uint32_t total_extension_count;
	vkEnumerateInstanceExtensionProperties(nullptr, &total_extension_count, nullptr);
	std::cout << "GPU support extension " + std::to_string(total_extension_count)<< std::endl;
	std::vector<VkExtensionProperties> vk_extension_properties(total_extension_count);
	vkEnumerateInstanceExtensionProperties(nullptr, &total_extension_count,vk_extension_properties.data());
	for (const auto& extension : vk_extension_properties)
	{
	...
	{
		std::cout << "create intance error" << std::endl;
	}
}
...
void CheckPhysicalDevice()
{
	...
	vkEnumeratePhysicalDevices(g_instance,&physical_device_count,nullptr);
	if (physical_device_count == 0)
	{
		std::cout << "no GPU find" << std::endl;
	}
	else
	{
		std::cout << "find GPU " << physical_device_count << std::endl;
	}
	std::vector<VkPhysicalDevice> physical_devices(physical_device_count);
	vkEnumeratePhysicalDevices(g_instance,&physical_device_count, physical_devices.data());
	for (const auto& physical_device : physical_devices)
	{
		...
		{
			std::cout << "find suitable physical device " << physical_device_property.deviceName << std::endl;
...
void CreateLogicalDevice()
{
	...
	//here is required queue index i set 0 directly
	...
	if (vkCreateDevice(g_physical_device, &device_create_info, nullptr, &g_device) != VK_SUCCESS)
	{
		std::cout << "logical device error" << std::endl;
	}
	else
	{
...
void ConnectNativeWindowHandle()
{
	VkWin32SurfaceCreateInfoKHR window_create_info{};
	window_create_info.sType = VK_STRUCTURE_TYPE_WIN32_SURFACE_CREATE_INFO_KHR;
	window_create_info.hwnd = glfwGetWin32Window(g_window);
	window_create_info.hinstance = GetModuleHandle(nullptr);
	
	if (vkCreateWin32SurfaceKHR(g_instance, &window_create_info, nullptr, &g_surface) != VK_SUCCESS)
	{
		std::cout << "window connect error" << std::endl;
	}
    ...
    vkGetPhysicalDeviceSurfaceSupportKHR(g_physical_device, 0, g_surface, &present_support);
    if (!present_support)
    {
        std::cout << "surface support error" << std::endl;
    }
}
...
private:
    ...
    void createGraphicsPipeline() {
        auto vertShaderCode = readFile("C:\\WorkSpace\\ShaderLab\\shader\\vert.spv");
        auto fragShaderCode = readFile("C:\\WorkSpace\\ShaderLab\\shader\\frag.spv");
